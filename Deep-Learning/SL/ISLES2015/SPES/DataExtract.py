...
#only no black ot slides are considered
...
dir_path = "D:\\Machine Learning\\data\\SPES_2015\\training"
...
np.save("./data/normal_data/training/imageNames",img_name)
#other data extract
...
for key in data_dict:
    ...
    for folder in tqdm(folders):
        ...
        sub_folders = os.listdir(os.path.join(dir_path,folder))
        for sub_folder in sub_folders:
            if 'penumbralabel' in sub_folder:
                sub_sub_folders = os.listdir(os.path.join(dir_path,folder,sub_folder))
                for sub_sub_folder in sub_sub_folders:
                    file_names = os.listdir(os.path.join(dir_path,folder,sub_folder,sub_sub_folder))
                    for each_file in file_names:
                        if ".nii" in each_file:
                            data = nib.load(os.path.join(dir_path,folder,sub_folder,sub_sub_folder,each_file))
                            data = data.get_fdata().T
                            if key in each_file:
